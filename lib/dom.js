...
NodeCache.prototype.popOne = function(hashCode) {
    if (this.cache[hashCode] == null) return null;
    return this.cache[hashCode].pop();
};
...
NodeCache.prototype.pop = function(hashCode) {
    var node = this.popOne(hashCode);
    if (node == null) return null;
    forEachChild(node, (child) => {
        this.pop(child.hashCode);
    });
    var par = node.parent;
    while (par != null) {
        this.popOne(par.hashCode);
        par = par.parent;
    }
    return node;
};
...
Creator.prototype._replaceOrCreate = function(node) {
    if (node.hashCode != null) {
        var cached = this.cache.pop(node.hashCode);
        if (cached != null) {
            node.dom = cached.dom;
...
Creator.prototype._updateChildren = function(node1, node2) {
    ...
    while (c1r != c1l && c2r != c2l) {
        c1l_next = c1l.next, c2l_next = c2l.next;
        if (c1l.hashCode != c2l.hashCode) {
            this._update(c1l, c2l);
            if (c2l.dom != c1l.dom) {
                dom.replaceChild(c2l.dom, c1l.dom);
            }
        } else {
            c2l.insertBefore(c1l);
            c2l.unlink();
    ...
    while (c2r != c2l) {
        ...
        dom.insertBefore(c2l.dom, c2r ? c2r.dom : null);
        c2l = c2l_next;
    }
    while (c1r != c1l) {
        c1l_next = c1l.next;
        dom.removeChild(c1l.dom);
        this.cache.push(c1l);
        c1l = c1l.next;
    }
};
